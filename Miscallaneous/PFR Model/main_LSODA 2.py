...
class MKModel:
    def __init__(self,Atomic_csv,Stoich_csv,Param_csv): #Inputs necessary to initialize the MK Model
        ...
        self.dplace,self.ODE_reltol,self.ODE_abstol = self.Tolerances()
    ...
    #------------------------------------------------------------------------------------------------------------------------------    
    def Tolerances(self, dplaces = 50, reltols = 1e-50, abstols=1e-40):
        self.dplace = dplaces      #Precision | Used for mp.dps in mpmath precision control
        self.ODE_reltol = reltols  #Relative tolerance
        self.ODE_abstol = abstols  #Absolute tolerance
        return self.dplace, self.ODE_reltol, self.ODE_abstol
    #------------------------------------------------------------------------------------------------------------------------------    
    ...
    def Coeff_extract(self):
       ...
       return Coeff
   #------------------------------------------------------------------------------------------------------------------------------
    def set_initial_coverages(self,init=[]): 
        # dplace = self.dplace
        mp.dps= self.dplace #int(dplace)
    ...
    def get_ODEs(self,t,cov,coverage=True): #t only placed for solve_ivp purposes #Functions used for calculating the rates of production
        ...
        if coverage==True:    
            ...
            return D
    #------------------------------------------------------------------------------------------------------------------------------      
    def solve_coverage(self,t=[],initial_cov=[],method='LSODA', Tf_eval=[],full_output=False,plot=False): #Function used for calculating (and plotting) single state transient coverages
        ...
        #Tolerances:
        reltol = self.ODE_reltol
        abstol = self.ODE_abstol
        solve = solve_ivp(self.get_ODEs,t_span,init,method,t_eval=T_eval,rtol=reltol,atol=abstol,vectorize=True,dense_output=full_output) #ODE Solver
