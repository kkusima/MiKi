        ...
        for j in np.arange(Nr):   #Looping through the reactions
            ...
            for i in np.arange(Ns):
                if self.Stoich.iloc[j,i+1]<0: #extracting only forward relevant rate parameters  #forward rxn reactants /encounter probability
                    fwd.append(np.power(matr[i],abs(self.Stoich.iloc[j,i+1])))
                    ...
                    rvs.append(np.power(matr[i],abs(self.Stoich.iloc[j,i+1])))   
...
class Fitting:    
    ...
    def curve_fit_func(self,method,maxfev,xtol,ftol):
        ...
        normox = np.where(y_values<1e-3,100,y_values)
        params, params_covariance = optimize.curve_fit(cost_function, x_values, y_values, sigma=normox
