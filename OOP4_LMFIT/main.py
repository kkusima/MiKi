...
class Fitting:    
    ...
    def covg_func(self,x,*fit_params):
        ...
        input_time=self.extract()[:,0]  #Used to set the time values to perform Numerical differentiation
        inp_init_covg = self.extract()[0,1:-1]  #Used to make sure the intial coverages match the input
    ...
    #------------------------------------------------------------------------------------------------------------------------------    
    def MKM_fit_func(self,Params,x,y):
        ...
        return np.reshape(soldat[:,1:],soldat[:,1:].size)
    #------------------------------------------------------------------------------------------------------------------------------    
    def MKM_cost_func(self,Params,x,y):
        #x = input times   ; y = input coverages (to be fitted)
        v = Params.valuesdict()
        fit_params_array = np.empty(len(self.k))
        for i in np.arange(len(self.k)):
        ...
        return np.reshape(soldat[:,1:],soldat[:,1:].size) - y
    #------------------------------------------------------------------------------------------------------------------------------    
    def MKM_custom_cost_func(self,Params,x,y):
        #x = input times   ; y = input coverages (to be fitted)
        v = Params.valuesdict()
        fit_params_array = np.empty(len(self.k))
        for i in np.arange(len(self.k)):
        ...
        sol,solt= self.solve_coverage(t=[0,x[-1]],initial_cov=inp_init_covg,Tf_eval=x,full_output=False) #Uses MKM.getODEs, but the inclass solve_coverage to add custom time dependancies
        soldat = np.insert(sol,0,solt,axis=1)   #Merging time and parameters
        ...
        for i in np.arange(rw):
            for j in np.arange(colmn):
                error_matrix[i,j]=(og[i,(j)] - soldat[i,j])**2
    ...
    #------------------------------------------------------------------------------------------------------------------------------     
    def minimize_fit(self,method='leastsq',weights_=None):
        values = np.array(self.extract()) # Input Data

        x_values = values[:,0] # Input Time variables (Independent Variable) (eg. KMC Time)
        y_values = np.reshape(values[:,1:],values[:,1:].size) #Normalized Input Dependent variable(s) (eg. KMC coverages)
        
        Mod = Model(self.MKM_cost_func)

        param_ = Mod.make_params()
        
        for i in np.arange(len(self.k)):
            param_.add('k'+str(i+1),value=self.k[i],min=0, max=1e4*self.k[i])

        result = Mod.fit(y_values, Params = param_, args=(x_values,y_values),method=method)
        enablePrint()
        print(param_)
        print(param_['k1'])
        print('---')
        # for pname, par in Mod.param_.items():
        #     print(pname, par)
        # blockPrint()
        vec_fitted_param = np.empty(len(self.k))
        for i in np.arange(len(self.k)):
            print(i)
            vec_fitted_param[i]= result.params['k'+str(i+1)].value
            print(vec_fitted_param)
        blockPrint()
        return vec_fitted_param
    #------------------------------------------------------------------------------------------------------------------------------     
    def minimize_fun(self,method,max_nfev = None):
        ...
        fitted_params = minimize(self.MKM_cost_func, parameters, args=(x_values,y_values), method=method, max_nfev = max_nfev)
    ...
    #------------------------------------------------------------------------------------------------------------------------------ 
    def minimize_custom_fun(self,method,max_nfev = None):
        values = self.extract() # Input Data

        x_values = values[:,0] # Input Time variables (Independent Variable) (eg. KMC Time)
        y_values = values[:,1:] # Input Dependent variable(s) (eg. KMC coverages)

        # parameters = self.PARAM   
        parameters = Parameters()
        for i in np.arange(len(self.k)):
            parameters.add('k'+str(i+1),value=self.k[i],min=0)     
        fitted_params = minimize(self.MKM_custom_cost_func, parameters, args=(x_values,values), method=method, max_nfev = max_nfev)
    ...
    def fitting_rate_param(self,option='min',plot=False,plot_norm=False,method_min='least_squares',method_cmin='least_squares',method_fit='leastsq',weights = None,mdl='MLPRegressor'
        ...
        og = self.extract(self.Input.to_numpy())
