...
class MKModel:
    ...
    def Coeff_extract(self,multipls = []):
        ...
        if self.BG_matrix == 'uniform':
            ...
            if isinstance(multipls, list) and not multipls:
    ...
    def set_Keq(self,Keq = [], Keq_setting = None):
        ...
        if self.Keq_setting == 'pseudo':
            ...
            if isinstance(Keq, list) and not Keq: #Checking if set to default
    ...
    def set_initial_coverages(self,init=[]): #empty sites included at the end of the code 
        ...
        if isinstance(init, list) and not init: 
    ...
    def get_rates(self,cov=[]): #cov = coverages  #Function used to calculate the rates of reactions
        ...
        if isinstance(cov, list) and not cov: #Condition to make sure default initial condition is what was set initially (from self.set_initial_coverages()) and if  different initial coverages are entered, they will be set as the default intial ccoverages
            cov = self.init_cov
        ...
        for j in np.arange(Nr):   #Looping through the reactions (except for the last one - for thermo consistency purposes)
            ...
            if j!=Nr-1:    #For calculating nonTCRC rate coefficients and corresponding rxn rates
    ...
    def solve_coverage(self,t=[],initial_cov=[],method='BDF',Tf_eval=[],full_output=False,plot=False): #Function used for calculating (and plotting) single state transient coverages
        ...
        if isinstance(t, list) and not t:  #Condition to make sure default time is what was set initially (from self.set_limits_of_integration()) and if a different time range is entered, it will be set as the default time limits of integration
            t=[self.Ti,self.Tf]  
        else:
        ...
        if isinstance(initial_cov, list) and not initial_cov: #Condition to make sure default initial condition is what was set initially (from self.set_initial_coverages()) and if  different initial coverages are entered, they will be set as the default intial ccoverages
            init = self.init_cov
        else:
        ...
        if isinstance(Tf_eval, list) and not Tf_eval: #Checking if set to default
            Tf_eval=None
        else:
            Tf_eval=Tf_eval
        ...
        reltol=self.rtol
        abstol=self.atol    
        solve = solve_ivp(self.get_ODEs,t_span,init,method,t_eval=Tf_eval,rtol=reltol,atol=abstol,dense_output=full_output) #ODE Solver
    ...
    def solve_rate_reaction(self,tf=None,Tf_eval=[],initial_coverage=[],plot=False): #Function used for calculating (and plotting) single state transient rates of reaction
        ...
        if isinstance(initial_coverage, list) and not initial_coverage: #Condition to make sure default initial condition is what was set initially (from self.set_initial_coverages()) and if  different initial coverages are entered, they will be set as the default intial ccoverages
            initial_coverage = self.init_cov
        else:
            initial_coverage = self.set_initial_coverages(initial_coverage)
        
        covg,covgt =self.solve_coverage(t=[self.Ti,tf],initial_cov=initial_coverage,Tf_eval=Tf_eval)
        rates_r = []
    ...
    def get_SS_rates_production(self,tf=None,Tf_eval=[]): #Function used for calculating the steady state rates of production
        ...
        rates_p,_ = self.solve_rate_production(tf=tf,Tf_eval=Tf_eval)  
    ...
    def get_X_RC_SS(self,p_inc=0.1,k_o_inp=[],rxn=-1):
        ...
        if isinstance(k_o_inp, list) and not k_o_inp: #Checking if set to default
    ...
    def periodic_operation_two_states(self,State1=[],State2=[],t1=None,t2=None,total_time=None,n_cycles=None,Initial_Covg=[],label='coverages',plot=False): #Function used for calculating (and plotting) the dynamic transient coverages
        ...
        if isinstance(State1, list) and not State1:
        ...
        if isinstance(State2, list) and not State2:  
        ...
        if isinstance(Initial_Covg, list) and not Initial_Covg: #Condition to make sure default initial condition is what was set initially (from self.set_initial_coverages()) and if  different initial coverages are entered, they will be set as the default intial ccoverages
            Initial_Covg = self.init_cov
        else:
            Initial_Covg = self.set_initial_coverages(Initial_Covg)
...
class Fitting:    
    ...
    #------------------------------------------------------------------------------------------------------------------------------    
    def extract(self,inp_array=[], InputType=[]): 
    ...
    def denormalize(self,Ext_inp_denorm=[]):
        ...
        return Denorm_inp
    #------------------------------------------------------------------------------------------------------------------------------    
    def TCRC_Enforcement(self,k=[]): #Function to calculate Thermodynamicaly Constrained Rate Coefficient and append to rate constant
        #The k represents all the rate constants in order of the reaction mechanism, excluding the last rate coefficient being calculated
        ###################### Thermodynamic constraint
        St_No = self.MKM.Stoichiometric_numbers
        Prod_N_1 = 1
        k = np.append(k,1.0) #Adding one number so that even rate coefficient
        kef = k
        ...
        rate_coeff_forward = k[-2]  #Calculating forward rate coefficient for the last step
    ...
    def covg_func(self,x,*fit_params):
        ...
        sol,solt= self.MKM.solve_coverage(t=[0,input_time[-1]],initial_cov=inp_init_covg,Tf_eval=input_time,full_output=False) #Uses MKM.getODEs, but the inclass solve_coverage to add custom time dependancies
        soldat = np.insert(sol,0,solt,axis=1)   #Merging time and parameters       
        #Norm_sol = self.normalize(Ext_inp=soldat) #(BECAUSE WE ARE NOT NORMALIZING)
    ...
    def rate_func_iCovg_iRates(self,x,*fit_params): #cost function for non-dynamic kmc #covg and instantaneous gasseous rates of prod
        ...
        kin_output = kin_output.astype(float) #converting from mpmath format to float for ease of implimentation in fitting packages like curvefit

        return kin_output  #Therefore important to note that kin_output here does not contain time
    #------------------------------------------------------------------------------------------------------------------------------    
    def error_func_0(self,fit_params):
        ...
        og =  np.insert(input_covg,0,input_time,axis=1)


        inp_init_covg = input_covg[0,:] 
        sol,solt= self.MKM.solve_coverage(t=[0,input_time[-1]],initial_cov=inp_init_covg,Tf_eval=input_time,full_output=False) #Uses MKM.getODEs, but the inclass solve_coverage to add custom time dependancies
        soldat = np.insert(sol,0,solt,axis=1)   #Merging time and parameters
       ...
       # max_covgs = np.zeros(colmn) #empty set to hold maximum coverages used to auto-select weight
        ...
        for i in np.arange(rw):
            for j in np.arange(colmn):
                error_matrix[i,j]=((og[i,(j+1)] - soldat[i,j+1])**2)/((og[i,(j+1)]+0.0001)**2)
    ...
    def error_func_1(self,fit_params):
        ...
        self.MKM.k = fit_params_array
        time_values,covg_values,ratep_values = self.extract()

        input_time=time_values
        inp_init_covg = covg_values
        sol,solt= self.MKM.solve_coverage(t=[0,input_time[-1]],initial_cov=inp_init_covg,Tf_eval=input_time) #Uses MKM.getODEs, but the inclass solve_coverage to add custom time dependancies
        soldat = np.insert(sol,0,solt,axis=1)   #Merging time and parameters
        #Norm_sol = self.normalize(Ext_inp=soldat)
    ...
    def minimizer_fit_func(self,method,gtol,ftol,maxfun,maxiter,tol,xatol,fatol,adaptive):
        ...
        y_values = np.reshape(covg_values,covg_values.size)
...
#---------# Bound generation method 2: Maintaining the order of magnitude of the guess values#---------#------#---------#---------                         
 ...
 #---------#---------#---------#---------#---------#---------#---------#---------#---------#---------#---------#---------#---------         
        ...
        #USING NUMDIFFTOOLS#---------#---------#---------#
        # @njit(fastmath=True,parallel=True) #wont work because of use of self may need to use jitclass see documentations
        def jacf(x):
    ...
    def ML_data_gen_0(self,n): ##degree of change is uniform across rates
        ...
        b = len(self.k)
        input_time,input_covg,input_rate = self.extract()
        og =  np.insert(input_covg,0,input_time,axis=1) #Original input
        ...
        for i in np.arange(n):
            ...
            sol,solt= self.MKM.solve_coverage(t=[0,input_time[-1]],initial_cov=inp_init_covg,Tf_eval=input_time)
            Covg[i,:,:] = sol
            # print(Rate_Coeff)
            # print(Covg)
    ...
    def ML_model_predict(self,Covg_fit,n,filename,mdl):
        ...
        if os.path.exists(filename) != True or n!=len(pd.read_excel('./'+filename, sheet_name='Rate_Coeffs')) or len(og) != len(pd.read_excel('./'+filename, sheet_name='Coverages')): #if the ML_data excel file doesnt exists or it doesn't have same no. of rows as the number specified, it replaces the dataset with the desired one
        ...
        if mdl == 'MLPRegressor':
            ...
            regr= KNeighborsRegressor(n_neighbors=8,weights='distance',p=2)
            ...
            regr = RandomForestRegressor(n_estimators=10000, criterion='poisson')  
    ...
    def fitting_rate_param(self,IGS = [], mask = 'OFF', masking_vec=[], option='cf',plot=False,plot_norm=False,method_cf='trf',method_min='L-BFGS-B',mdl='MLPRegressor'
        ...
        #Extracting the original input data:
        if self.Input_Type=='iCovg_iRates':
            og_time,og_covg,og_rate_p = self.extract() #Note: og_covg here contains time
        ...
        if option=='cf':
            ...
            if self.Input_Type=='iCovg_iRates':
                kin_output = self.rate_func_iCovg_iRates(x_values, *params) #kinetic output from running predicted rate parameters; This kinetic output differs based on what kind of input was given
            ...
            converg = np.sqrt(np.diag(params_covariance)) #Variance
        ...
        time = og_time    # OG time values 
        covg_og = og_covg  # OG coverages
        ...
        ####Printing out the COVARIANCE MATRIX
        print('\n \033[1m' + 'Covariance Matrix: \n'+ '\033[0m')
        print(params_covariance)

        ####Printing out the CONDITION NUMBER
        print('\n \033[1m' + 'Condition Number: \n'+ '\033[0m')
        print('-> The condition number for the above covariance matrix:\n',np.linalg.cond(params_covariance))
